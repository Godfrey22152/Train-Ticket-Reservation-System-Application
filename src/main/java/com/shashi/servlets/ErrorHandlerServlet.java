...
public class ErrorHandlerServlet extends HttpServlet {
    ...
    public void service(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
        ...
        String buttonHtml = "<button onclick=\"history.back()\">Return to Previous Page</button>";
        if (statusCode >= 500) {
            buttonHtml = "<button onclick=\"history.back()\">Reload Page</button>";
        }
        ...
        if (statusCode == 401) {
            ...
            pw.println("<p class='menu' style='color:red'>" + errorCode + "</p><br>");
            pw.println("<p class='menu'>" + errorMessage + "</p>");
            pw.println(buttonHtml);
            pw.println("</div>");
        }
    }
}
